  ...
  <div>
    ...
    <ul v-show="hasItems">
      ...
      <li v-for="(item, $item) in items" :class="activeClass($item)" @mousedown="hit" @mousemove="setActive($item)">
        <span v-text="item.NameLast + ', ' + item.NameFirst + ' Rm: ' + item.Room "></span>
      </li>
    </ul>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    onHit (item) {
      console.log(item);
      //alert(item._id);
      if(this.clickaction == "fillinput"){
        console.log(item.NameLast);
        return typeaheadCallback(item.NameLast);
